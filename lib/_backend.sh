...
backend_redis_create() {
  print_banner
  printf "${WHITE} 💻 Criando banco MySQL e phpMyAdmin via Docker...${GRAY_LIGHT}"
  printf "\n\n"
  ...
  docker run --name mysql-${instancia_add} \
                -e MYSQL_ROOT_PASSWORD=${mysql_root_password} \
                -e MYSQL_DATABASE=${instancia_add} \
                -e MYSQL_USER=${mysql_root_user} \
  ...
  docker run --name phpmyadmin-${instancia_add} \
                -d --link mysql-${instancia_add}:db \
                -p ${phpmyadmin_port}:80 phpmyadmin/phpmyadmin
EOF
sleep 2

ufw allow ${phpmyadmin_port} #allow port phpMyAdmin
